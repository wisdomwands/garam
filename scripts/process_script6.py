...
# Using gpt-5 as requested in previous similar tasks (inheriting from context)
MODEL_NAME = "gpt-5" 
...
def refine_text(text, context_content):
    ...
    4. **êµ¬ì¡°??*: ?¸ì?ì¸ ?ì£¼?ë ëª©ì°¨ë¥?ë§ë¤ì§ ë§ê³ , ?ë¬¸??ë¬¸ë§¥???°ë¼ ë¬??ë¥´???ì°?¤ë½ê²??´ì´ì§?ë¡ ?ì ?ì¸?? ë¬¸ë¨? ?´ì©???ë¦??ë°ë????ì ???ë?´ì£¼?¸ì.
...
def main():
    ...
    source_dir = os.path.join(base_dir, "?ì£¼?ë§?.json", "2013") # Modified source
    target_dir = os.path.join(base_dir, "?ì£¼?ë§?.doc", "2013-2") # Modified target
